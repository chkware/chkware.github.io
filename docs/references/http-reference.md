...
`variables` is a top-level block that defines local variables. These variables are file scoped. How to write a `variables:` block is already defined in [*variables*](/docs/references/variables).
